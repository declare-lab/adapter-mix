...
class VarianceAdaptor(nn.Module):
	...
	def __init__(self, preprocess_config, model_config, train_config, d_model):
		...
		self.use_adapter = model_config["adapters"]["variance_adapter"]["required"]
		self.d_model = model_config["transformer"]["decoder_hidden"]
		self.r = model_config["adapters"]["variance_adapter"]["r"]
	...
	def forward(
		...
		residual = x
		if self.use_adapter:
			x = self.adapter_block(x,residual)
