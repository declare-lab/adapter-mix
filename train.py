...
from utils.sampler import DistributedBatchSamplerSimilarLabel, BatchSamplerSimilarLabel
...
def freeze_exclude_praram(model,config):
	for name, param in model.named_parameters():
		if any(substring in name for substring in config["adapters"]["unfreeze_parameters"]):
			pass
...
def train(rank, args, configs, batch_size, num_gpus):
    ...
    dataset = Dataset(
        "train_{}.txt".format(dataset_tag), preprocess_config, model_config, train_config, sort=False, drop_last=True
    )

    # data_sampler = DistributedSampler(dataset) if num_gpus > 1 else None
    data_sampler = DistributedBatchSamplerSimilarLabel(dataset=dataset,batch_size=train_config["optimizer"]["batch_size"],rank = rank, drop_last=True) if num_gpus > 1 else BatchSamplerSimilarLabel(dataset=dataset,batch_size=train_config["optimizer"]["batch_size"])
    ...
    if model_config["adapters"]["freeze_basemodel"]:
        freeze_exclude_praram(model,model_config)
